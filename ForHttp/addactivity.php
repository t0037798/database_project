	  ...
	  <div class="col-xs-7 col-sm-7">
		...
		<?php if(isset($_SESSION['Account'])) : ?><!--假如有留下session-->
			<?php if($row_S[0]): ?><!--假如是管理員-->
				...
				<form name="form" method="post" action="newactivity.php" enctype="multipart/form-data">
					<div class="form-group">
						<label for="exampleInputEmail1">活動名稱</label>
						<input type="text" class="form-control" name="ActTitle" placeholder="請輸入活動名稱">
					</div>
					<div class="form-group">
						<label for="exampleInputEmail1">活動地點</label>
						<input type="text" class="form-control" name="ActLocation" placeholder="請輸入活動地點">
					</div>
					<div class="form-group">
						<label for="exampleInputEmail1">活動內容</label>
						<textarea class="form-control" rows="8" name="ActContent" placeholder="輸入活動內容"></textarea>
					</div>
					<div class="form-group">
						<label for="exampleInputFile">上傳活動照片</label>
						<input type="file" id="exampleInputFile" name="myfile">
						<p class="help-block">請上傳jpg、jpeg、png等圖檔，別給我亂傳。</p>
					 </div>
					</br>
					<div style="text-align:center"><button type="submit" class="btn btn-primary">發送</button></div>				  		  
				</form>	
